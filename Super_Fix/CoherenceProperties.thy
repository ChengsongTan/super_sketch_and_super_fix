...
definition testing_P :: "Type1State \<Rightarrow> bool" where [simp]:
  "testing_P T = ( (SWMR T) \<and>
    C_msg_P_oppo ISD nextHTDDataPending (\<lambda>T i. \<not> CSTATE Modified T i) T \<and> 
    H_msg_P_same SD nextDTHDataPending (\<lambda>T i. \<not> CSTATE Modified T i) T \<and> 
    H_msg_P_same SAD nextDTHDataPending (\<lambda>T i. \<not> CSTATE Modified T i) T)"
