...
\<close>
ML \<open>
(* Helper function for two-step try strategy: try0 first, then sledgehammer *)
fun try_strategies_two_step st =
  let
    val sol1 = (case Try0.try0 (SOME (Time.fromSeconds 30)) [] st of 
      [] => "sorry"
      | {command, ...} :: _ => command
    );
    val sol2 = if sol1 = "sorry" then
...
(* Generate solving text for sub-subgoals with two-level splitting *)
fun generate_two_level_solving_text i state msplit_ref msplit_txt mreduce_ref mreduce_txt = 
  let 
    val state_subgoaled = (#2 o Subgoal.subgoal_cmd Binding.empty_atts NONE (false, [])) (Proof.prefer i state);
    val state_splitted = (case msplit_ref of (SOME m) =>  (Seq.the_result "" (Proof.apply m (state_subgoaled)))
                                         | NONE => state_subgoaled );
    ...
    val _ = writeln ("sub_subgoals_num: " ^ Int.toString sub_subgoals_num);
    val string_pairs_list = (if sub_subgoals_num > 0 then (Par_List.map (fn j => 
      let 
        val statej = Proof.prefer j state_reduced;
        val result = try_strategies_two_step statej;
      in ("success", result) end) (createList(1, sub_subgoals_num + 1))) else []);
    val proof_text_list = map snd string_pairs_list;
    val done_or_nil = (if sub_subgoals_num = 1 then [] else [" done\n"]);
    val prefix_apply = if msplit_txt = "-" then "" else " apply " ^ msplit_txt;
    val prefix_reduce = if mreduce_txt = "-" then "" else " apply " ^ mreduce_txt;
    val text = cat_lines ([prefix_apply ^ prefix_reduce] @ proof_text_list @ done_or_nil);
  in text end;

(* Print Isar skeleton for try_sketch2 with two-level splitting *)
fun print_try_sketch2_isar_skeleton ctxt indent keyword stmt i state msplit_ref msplit_txt mreduce_ref mreduce_txt =
  let
    val ((fixes, assms, concl), ctxt') = eigen_context_for_statement stmt ctxt;
    val prefix = replicate_string indent " ";
    val prefix_sep = "\n" ^ prefix ^ "    and ";
    ...
    val message1 = generate_two_level_solving_text i state msplit_ref msplit_txt mreduce_ref mreduce_txt;
    val s = cat_lines ([show_s] @ map_filter I [if_s, for_s] @
      [ prefix ^  "  " ^ (if is_none if_s then "" else "using that ") ^ message1]);
  in
    s
...
(* Print try_sketch2 proof structure *)
fun print_try_sketch2 ctxt method_text1 clauses state msplit_ref msplit_txt mreduce_ref mreduce_txt =
  let 
    val n = subgoal_count state;
    val t_start = Timing.start ();
    val s = "proof" ^ method_text1 :: Par_List.map (fn (stmt, i) => 
      print_try_sketch2_isar_skeleton ctxt 2 "show" stmt i state msplit_ref msplit_txt mreduce_ref mreduce_txt) 
      (ListPair.zip (clauses, createList'(n))) @ ["qed"];
...
(* Main function for try_sketch2 *)
fun print_proof_try_sketch2 format opt_mrange_toks1 opt_mrange_toks2 state = 
  let
    val (opt_m1, m1_txt) = case opt_mrange_toks1 of 
      SOME (m,toks) => (SOME m, coalesce_method_txt (map Token.unparse toks))
      | NONE => (NONE, "-");
    val (opt_m2, m2_txt) = case opt_mrange_toks2 of 
      SOME (m,toks) => (SOME m, coalesce_method_txt (map Token.unparse toks))
      | NONE => (NONE, "-");
      
    val prf_st = if is_some opt_m1
      then state
        |> Toplevel.proof_of
        |> Proof.proof opt_m1
        |> Seq.the_result ""
    ...
    val configs = [show_markup, Printer.show_type_emphasis, show_types, show_sorts, show_consts];
    val ctxt' = fold (fn bconf => Config.put bconf false) configs ctxt;
    val _ = Output.tracing "Producing goals for try_sketch2..."
    val (count, clauses) = map split_clause (Logic.strip_imp_prems (Thm.prop_of goal))
      |> enum_count_from 1;
    val _ = Output.tracing ("Produced " ^ Value.print_int count ^ " goals with two-level splitting.")
    
    val final_texts = (case format of
      LEMMAS (read_path, write_dir) => map (sketch_as format ctxt (init_indent_of format)) clauses
      | SHOW_IFS => 
        let
          val _ = Output.tracing "Making proof skeleton with two-level splitting..."
          val lines = print_try_sketch2 ctxt' (" " ^ m1_txt) clauses prf_st opt_m2 m2_txt NONE "-";
        in lines end
    )
  in Library.space_implode "" final_texts end;
\<close>
