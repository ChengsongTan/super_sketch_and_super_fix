theory TSingle imports  "../Super_Sketch/Super_Sketch"  AllBackgroundInvariants  begin
...
definition first_four_hundred: "first_four_hundred T = (    (nextSnpRespIs RspSFwdM T 1 \<longrightarrow> CSTATE Shared T 1 \<or> CSTATE SMAD T 1 \<or> CSTATE SIA T 1 \<or> CSTATE SIAC T 1) \<and>
...
definition single: "single T =     ((HSTATE MA T \<and> nextSnpRespIs RspIHitSE T 0 \<longrightarrow> \<not> nextReqIs DirtyEvict T 0) \<and>
    (HSTATE MA T \<and> nextSnpRespIs RspIHitSE T 1 \<longrightarrow> \<not> nextReqIs DirtyEvict T 1)) "

thm SWMR_state_machine_def
(*800 conjuncts * 70 rules*)
theorem all_transitions_preserve_P  : assumes
  "SWMR_state_machine T" shows " \<forall>T' \<in> set (concat (map (\<lambda> transition. transition T 0) allTransitions' )).  single T'"
proof -
...
have i977: "(HSTATE SharedM T \<and> CSTATE SIAC T 0 \<and> nextGOPendingIs GO T 0 \<and> nextGOPendingState Invalid T 0 \<longrightarrow> \<not> CSTATE IIA T 1) " by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
  ...
  show ?thesis
  unfolding  SWMR_state_machine_def single
