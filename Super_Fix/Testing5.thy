theory Testing5 imports  "../Super_Sketch/Super_Sketch"  AllBackgroundInvariants  begin
...
theorem all_transitions_preserve_P  : assumes
  "SWMR_state_machine T" shows " \<forall>T' \<in> set (concat (map (\<lambda> transition. transition T 0) allTransitions' )).  first_hundred T'"
proof -
...
have i977: "(HSTATE SharedM T \<and> CSTATE SIAC T 0 \<and> nextGOPendingIs GO T 0 \<and> nextGOPendingState Invalid T 0 \<longrightarrow> \<not> CSTATE IIA T 1) " by (insert assms, unfold SWMR_state_machine_def, elim conjE, assumption)
  ...
  show ?thesis
  unfolding first_hundred SWMR_state_machine_def
