...
theory Meta_Sketch
  ...
  keywords "sketch" "explore" "sketch_subgoals" "super_sketch" "super_sketch3" "super_sketch2b" "double_sketch" "double_sketch4" "meta_sketch" :: diag
begin
...
val _ =
  Outer_Syntax.command \<^command_keyword>\<open>meta_sketch\<close>
    ...
    ((Scan.repeat (Scan.trace Method.parse)        (* methods 列表：取第一个当 user method *)
     ...
     >> (fn ((((meths, msimp), adsimp), rescue), first_split_add_rescue) =>
       Toplevel.keep_proof (fn state =>
         let
           val pstate = Toplevel.proof_of state
         in
